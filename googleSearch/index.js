...
function onSuggestionResponse(data) {
  const suggestionsElement = document.getElementsByClassName(
    "search__suggestions__list"
  )[0];
  let suggestionHTML = "";
  ...
  for (const suggestion of data) {
    suggestionHTML += createSuggestionElement({
...
function onNewInput(event) {
  if (searchInput.value) {
    api.get(HOST + "/autocomplete", searchInput.value, onSuggestionResponse);
  } else {
...
function getAutocompleteHandler(data) {
  ...
  const result = [];
  while (result.length < NUM_AUTOCOMPLETE_RESULTS) {
    const suggestion = generateSuggestion(data);

    if (result.find((result) => result.suggestion === suggestion)) continue;
